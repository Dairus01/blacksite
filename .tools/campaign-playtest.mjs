...
try {
 ...
 for(let mission=1;mission<=4;mission++){
  ...
  if(mission===2){const opening=await state();assert.equal(opening.enemyCount,2,'refinery opening patrol appears at deployment');assert.ok(opening.enemies.some(e=>Math.hypot(e.pos[0]-opening.pos[0],e.pos[1]-opening.pos[1])<10),'opening patrol is visible down the first lane');}
  ...
  for(let n=0;n<160;n++){
   ...
   else {const target=st.objectivePosition;const ok=await page.evaluate(pos=>window.game.debug.teleportPlayer(pos),target);assert.ok(ok,`objective ${st.objective} reachable at ${target}`);await page.keyboard.press('KeyE');await page.waitForTimeout(350);if(mission===2&&[1,2].includes(st.stage)){const response=await state();assert.equal(response.stage,st.stage+1,'relay activates on approach');assert.equal(response.enemyCount,2,'relay triggers its own visible Sentinel response');await page.screenshot({path:path.join(out,`refinery-relay-${st.stage}-response.png`)});}}
